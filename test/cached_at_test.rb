...
class CachedAtTest < ActiveSupport::TestCase
  ...
  test "#cache_key" do
    ...
    assert_equal "cached_at_test/accounts/#{account.id}-#{t1.utc.to_s(:usec)}", account.cache_key_including
    assert_equal "cached_at_test/accounts/#{account.id}+b4c1948c087fafc89a88450fcbb64c77@#{t2.utc.to_s(:usec)}", account.cache_key_including(:organization)
    assert_equal "cached_at_test/accounts/#{account.id}+b471431f7777fe57283f68842e724add@#{t3.utc.to_s(:usec)}", account.cache_key_including(organization: :images)
    assert_equal "cached_at_test/accounts/#{account.id}+b471431f7777fe57283f68842e724add@#{t3.utc.to_s(:usec)}", account.cache_key_including([organization: [:images]])
  end
